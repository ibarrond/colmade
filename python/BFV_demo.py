...
# n1, n2 = 15, -5
n1, n2 = randint(-(2**15),2**15-1), randint(-(2**15),2**15-1)//2*2
...
def Decryption(self, ct, m2):
    ...
    # encoded mask is m2
    m = ct[1]*self.sk + ct[0]
    m.F = [((self.t*x)/self.q + y) for x, y in zip(m.F, m2.F)]
    m = round(m)
    m = m % self.t
...
m1_masked = Decryption(Evaluator, ct1, m2)
